...
const InsightsTemplate: React.FC = () => {
  ...
  return (
    <PageLayout>
      {blog && <img className="py-6 w-full" src={`http://localhost:1337${blog?.attributes.picture.data.attributes.url}`}></img>}
      ...
      {blog && <div className="flex flex-col lg:flex-row py-6">
        ...
        <div className="grid-blog">
          ...
          {<div className="col-start-1 row-start-4 flex-col">
            {blog.attributes.content.map((d, i) => <div key={i} className={classnames({ "blog-subheading": d.type == "heading" }, { "blog-paragraph": d.type == "paragraph" })}>{d.children[0].text}</div>)}
          </div>}
        </div>
      </div>}
    </PageLayout>
