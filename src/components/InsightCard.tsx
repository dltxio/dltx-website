...
const InsightCard: React.FC<InsightShort & { showImage?: boolean }> = (insight) => {
    return (<a key={insight.attributes.slug} href={`/insights/${insight.attributes.slug}`}>
        <motion.div className={classnames("w-full bg-gradient-to-r from-transparent to-[#202020] rounded-xl overflow-hidden hover:text-white", { "lg:aspect-[4/3]": !insight.showImage }, { "lg:aspect-[3/4]": insight.showImage })}
            ...
            {insight.showImage && <img className="h-1/2 w-full object-cover" src={`http://localhost:1337${insight.attributes.picture.data.attributes.url}`}></img>}
